...
export const api = {
  me: (signal?: AbortSignal) => json<MeResponse>("/api/me", { signal, cache: "no-store" }),
  ...
  sessionSummaries: (q = "", status = "", signal?: AbortSignal) =>
    json<SessionSummaryListResponse>(`/api/session-summaries?q=${encodeURIComponent(q)}&status=${encodeURIComponent(status)}`, { signal }),
