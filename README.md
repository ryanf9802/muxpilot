...
The target branch must already exist locally. An agent can change its session target through the bundled workflow only after naming the `fixed-target` guard, explaining the integration consequence, and receiving separate explicit confirmation. An active task worktree is preserved, then rebased if necessary and revalidated before integration. Multiple tasks can target the same branch concurrently; only their brief final integration operations serialize, so completion order determines landing order. Muxpilot never automatically pulls or pushes.
