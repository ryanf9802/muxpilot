...
For Git repositories, muxpilot runs `codex` from a neutral control directory and creates no checkout until the agent begins a change task. The agent then creates a meaningfully named private branch/worktree, commits and tests there, runs an independent structured review/fix loop, atomically fast-forwards a shared muxpilot-managed target ref, and removes the worktree after integration. Missing sessions preserve unfinished work in private branches and recovery refs without retaining a checkout. The original checkout may remain on any branch and may be dirty, including the named target branch. Independent reviews are capped at five minutes; if one does not complete, the agent halts and asks before using the explicit unreviewed-integration override. The compact session Git panel shows managed/remote status and is the only place that can explicitly push the result to the named remote branch.
