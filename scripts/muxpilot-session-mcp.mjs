...
function toolDefinitions() {
  ...
  return [
    ...
    { name: "read_session_runtime", description: "Read neutral runtime, service, socket, resource-unit, and attachment evidence for a session.", inputSchema: object({ sessionId }, ["sessionId"]) },
    { name: "read_session_process_tree", description: "Read direct process-tree evidence for either a tmux or app-server session.", inputSchema: object({ sessionId }, ["sessionId"]) },
    { name: "read_session_protocol_journal", description: "Read bounded raw app-server protocol journal bytes for a session.", inputSchema: object({ sessionId, offset: { type: ["integer", "null"], minimum: 0 }, length: { type: "integer", minimum: 1, maximum: 262144 } }, ["sessionId"]) },
    ...
    { name: "send_message", description: "Send a message to any live muxpilot session. Descendant budget and context guards apply.", inputSchema: object({ sessionId, text: { type: "string", maxLength: 200000 }, mode: { type: "string", enum: ["default", "plan"] }, allowHighContext: { type: "boolean" }, reason: { type: "string" } }, ["sessionId", "text"]) },
