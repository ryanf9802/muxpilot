...
loadDotenv();
applyDevRuntimeDefaults();

const HEALTH_CHECK_TIMEOUT_MS = 5000;
const backendPort = validPort(process.env.MUXPILOT_PORT, 4177);
const webPort = validPort(process.env.MUXPILOT_WEB_PORT, 5177);
...
if (backendActive && webActive) {
  console.log(`Development server is already active. Reusing ${webUrl}.`);
  process.exit(0);
}
...
if (backendPortOccupied && !backendActive) {
  ...
  console.error("Not starting a duplicate backend. Reuse the existing process if it is intentional, or run pnpm dev:stop before pnpm run:dev.");
  process.exit(1);
}

if (webPortOccupied && !webActive) {
  console.error(`Frontend port ${webPort} is already in use, but ${webUrl} did not respond.`);
  console.error("Not starting a duplicate frontend. Reuse the existing process if it is intentional, or run pnpm dev:stop before pnpm run:dev.");
  process.exit(1);
}

if (backendActive) {
  console.log(`Backend is already active on ${backendUrl}; starting frontend on ${webUrl}.`);
  await runPnpm(["--filter", "@muxpilot/web", "dev"]);
} else if (webActive) {
  console.log(`Frontend is already active on ${webUrl}; starting backend on ${backendUrl}.`);
  await runPnpm(["--filter", "@muxpilot/server", "dev"]);
} else {
  console.log(`Starting development server at ${webUrl} with backend ${backendUrl}.`);
  await runPnpm(["--parallel", "--filter", "@muxpilot/server", "--filter", "@muxpilot/web", "dev"]);
}
...
function applyDevRuntimeDefaults() {
  ...
  process.env.MUXPILOT_WEB_PORT ??= "5177";
  process.env.MUXPILOT_DATA_DIR ??= "./data/dev";
  process.env.MUXPILOT_DB_PATH ??= "./data/dev/muxpilot.db";
}
