...
For authenticated ChatGPT accounts, the Codex usage panel shows the current limit windows, earned usage-reset tokens and their expiration details, and daily token activity over the past 30 days. Using a reset token requires confirmation. If the result is interrupted, the panel preserves the attempt so **Retry** checks the same idempotent redemption rather than consuming another token. Refresh reloads account limits, reset tokens, and token activity directly from Codex.
