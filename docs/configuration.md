...
- `MUXPILOT_HEAVY_QUEUE_ENABLED`, `MUXPILOT_HEAVY_COMPLETION_ENABLED`, and `MUXPILOT_HEAVY_RUN_ID` bind a helper invocation to the managed heavyweight scheduler and completion handoff. Completion handoff is enabled only when user-systemd session scopes are available.
