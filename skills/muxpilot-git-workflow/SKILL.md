...
5. Follow repository guidance, including any required full build gate, but otherwise default to focused file/module lint, typechecking, and tests. A repository-required build is authorized validation for that repository and must run before integration. Do not treat the same-agent self-review step below as a PR-style review. Run other repository-wide scans or test suites only when the user explicitly requests them, or when the user explicitly requests a PR-style review of a branch or ref.
...
8. Review the complete target-to-task diff yourself. Fix every actionable finding, rerun affected focused checks and any repository-required build, commit fixes, and review again. Repeat until a final review finds nothing actionable. Any material change invalidates the prior review and required build result.
9. Run `node <helper-dir>/muxpilot-git-finish.mjs`. If the target advanced, the helper rebases and stops; rerun affected focused checks, any repository-required build, and the complete self-review loop before retrying. Resolve conflicts in the task worktree, then do the same.
10. Report completion only after the helper prints `INTEGRATED`. Identify the integrated commit and the focused checks and repository-required build that succeeded. In managed sessions the final validated fast-forward, target-checkout update, and cleanup use the authenticated local muxpilot broker; older or unmanaged helpers retain direct local integration. Successful integration removes the worktree and temporary branch. Failed or unfinished work is preserved.
