...
10. Whenever the user explicitly directs integration without review, run `node "$CODEX_HOME/skills/muxpilot-git-workflow/scripts/muxpilot-git-finish.mjs" --integrate-without-review`. The explicit flag bypasses review regardless of whether review has not run, failed, or returned findings. Never use this flag without explicit user direction.
